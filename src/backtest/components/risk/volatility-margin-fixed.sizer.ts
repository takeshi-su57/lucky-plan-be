...
export interface VolatilityMarginFixedParams {
  baseMargin: number; // Base margin in USDT (used when volatility = baseline)
...
/**
 * Component metadata for Volatility-Adjusted Margin (Fixed Base) Sizer
 */
export const volatilityMarginFixedMeta: ComponentMeta = {
  name: 'Volatility-Adjusted Margin (Fixed Base)',
  description:
    'Fixed base margin adjusted by volatility with fixed leverage. High volatility = smaller margin, low volatility = larger margin.',
  params: [
    {
      name: 'baseMargin',
      type: 'number',
      required: true,
      min: 100,
      description: 'Base margin in USDT when volatility equals baseline',
    },
    {
      name: 'minMargin',
      type: 'number',
...
/**
 * Volatility-Adjusted Margin (Fixed Base) Sizer
 *
 * Uses a fixed base margin that adapts to market volatility with fixed leverage.
 ...
 */
export class VolatilityMarginFixedSizer implements PositionSizer {
  readonly name = 'volatilityMarginFixed';
  ...
  constructor(params: VolatilityMarginFixedParams) {
    this.baseMargin = params.baseMargin;
  ...
  /**
   * Calculate volatility-adjusted margin
   */
  private calculateAdjustedMargin(): number {
    const currentAtr = this.atr.getATR();
    const atrSma = this.getAtrSma();

    // If not enough data, use base margin
    ...
    const rawMargin = this.baseMargin * volatilityRatio;
  ...
  calculateSize(
    ...
    const requestedMargin = this.calculateAdjustedMargin();
  ...
  getCurrentMargin(): number {
    return this.calculateAdjustedMargin();
  }
}
