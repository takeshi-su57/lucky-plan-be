...
export interface VolatilityLeverageFixedParams {
  fixedMargin: number; // Fixed margin in USDT
...
/**
 * Component metadata for Volatility-Adjusted Leverage (Fixed Margin) Sizer
 */
export const volatilityLeverageFixedMeta: ComponentMeta = {
  name: 'Volatility-Adjusted Leverage (Fixed Margin)',
  description:
    'Fixed margin with leverage adjusted by volatility. High volatility = lower leverage, low volatility = higher leverage.',
  params: [
    {
      name: 'fixedMargin',
      type: 'number',
...
/**
 * Volatility-Adjusted Leverage (Fixed Margin) Sizer
 *
 * Uses a fixed margin with leverage that adapts to market volatility.
 ...
 */
export class VolatilityLeverageFixedSizer implements PositionSizer {
  readonly name = 'volatilityLeverageFixed';

  private fixedMargin: number;
  ...
  constructor(params: VolatilityLeverageFixedParams) {
    this.fixedMargin = params.fixedMargin;
  ...
  calculateSize(
    ...
    const requestedMargin = this.fixedMargin;
    ...
    const notionalValue = actualMargin * leverage;
