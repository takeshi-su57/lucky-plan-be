...
query ListTemplates($category: StrategyCategory, $isActive: Boolean, $limit: Int, $offset: Int) {
  strategyTemplates(category: $category, isActive: $isActive, limit: $limit, offset: $offset) {
    id
    name
    description
    category
...
export function useSearchSubscription(onUpdate: (search: TemplateSearch) => void) {
...
useSearchSubscription((updatedSearch) => {
  // Update local state
  setSearches(prev =>
    prev.map(s => s.id === updatedSearch.id ? { ...s, ...updatedSearch } : s)
  );
